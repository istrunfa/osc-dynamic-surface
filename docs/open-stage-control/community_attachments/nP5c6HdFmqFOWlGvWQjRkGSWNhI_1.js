...
const { handleSelectorMessage } = require('./modules/selectorModule.js');
const { handleTransposeMessage } = require('./modules/transposeModule.js');
const { handleFilterMessage } = require('./modules/filterModule.js');
const { handleNudgeMessage } = require('./modules/nudgeModule.js');
...
app.on('sessionOpened', () => {
    ...
    /*
    These bits are not implemented yet
    console.log(`Loading Custom Module version ${customModuleVer}...`);

    // Set Expression Map update button to unprimedText
    resetExpMapRebuildBtn();

    // Set root key as not set
    const setKeyTextLabelTxt = "Not Set";
    addrootKeyLabel(setKeyTextLabelTxt);
    */
});
...
module.exports = {
    ...
    oscOutFilter: function (data) {
        ...
        const handlers = {
            "/testBtn": () => {
                console.log("Test Button Pressed again 3");
                let testThis = getStartUpConfig.testThis;
            },
            "/ensFilterBtn": handleFilterMessage,
            "/LibraryFilterBtn_": handleFilterMessage,
            "/InstrFilterBtn": handleFilterMessage,
            "/folderGetBtn": handleFilterMessage,
            "/soloOnlyFilterSwitch": handleFilterMessage,
            "/instrFilterSwitch": handleFilterMessage,
            "/ensFilterSwitch": handleFilterMessage,
            "/LibraryFilterSwitch": handleFilterMessage,
            "/noteSelector": handleSelectorMessage,
            "/panel": handleSelectorMessage,
            "/selector_selectNote": handleSelectorMessage,
            "/transDirection": handleTransposeMessage,
            "/transpose": handleTransposeMessage,
            "/transposeInterval": handleTransposeMessage,
            "/nudge": handleNudgeMessage,
